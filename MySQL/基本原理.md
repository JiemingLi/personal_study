...
![img](https://img.jacian.com/note/img/20210606213605.jpg)
...
![Content is described in the surrounding text.](https://dev.mysql.com/doc/refman/5.6/en/images/innodb-buffer-pool-list.png)
...
在进行DML的操作时候，如果BP没有对应的数据，不会立刻把数据从磁盘加载到Buffer Pool，而是在Change Buffer中缓冲变更，等后续数据读取的时候再把变更和数据合并到Buffer Pool

> 仅适用于非唯一性索引：如果是唯一索引，innodb在更新数据库的时候会查询一次数据库，进行唯一性
...
![image.png](https://segmentfault.com/img/bVcQJxC)
...
事务提交后，使用的undo日志不再使用，需要Purge thrad回收分配好的页（缓冲区和磁盘的页）
...
![ibd_arch](https://img2020.cnblogs.com/blog/1861029/202106/1861029-20210604101356850-1096573215.jpg)
...
![MVCC](http://ww1.sinaimg.cn/large/005SjMHzly1gq42bai7jtj31bu0hmtcg.jpg)

### redo log & binlog

Redo Log：指事务中修改的任何数据，将最新的数据备份存储的位置（Redo Log），被称为重做日志。
Redo Log 的生成和释放：随着事务操作的执行，就会生成Redo Log，在事务提交时会将产生 Redo Log写入
Log Buffer，**不是**随着事务的提交就立刻写入磁盘文件。等事务操作的脏页写入到磁盘之后，Redo Log的
使命也就完成了，Redo Log占用的空间就可以重用（被覆盖写入）。
...
![redo log工作原理](http://ww1.sinaimg.cn/large/005SjMHzly1gq42goe7c1j319q0wo430.jpg)
...
![写入机制](http://ww1.sinaimg.cn/large/005SjMHzly1gq43vnhxomj31900g2ta1.jpg)write pos 和 checkpoint 之间还空着的部分，可以用来记录新的操作。如果 write pos 追上checkpoint，表示写满，这时候不能再执行新的更新，得停下来先擦掉一些记录，把 checkpoint推进一下。
...
![redo log 参数](http://ww1.sinaimg.cn/large/005SjMHzly1gq443c8n6cj319y0ho0wt.jpg)
